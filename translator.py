...
class TranslatorApp:
    def __init__(self, root):
        ...
        # ---------- 使用 grid 布局主区域 ----------
        root.grid_rowconfigure(2, weight=1)   # 模组列表区域可扩展
        root.grid_rowconfigure(7, weight=1)   # 日志区域可扩展
        root.grid_columnconfigure(0, weight=1)  # 唯一列占满宽度

        # ---------- 第0行：添加文件、添加文件夹、清空列表按钮 ----------
        ...
        inner_canvas.grid(row=0, column=0, sticky='nsew')
        inner_scrollbar.grid(row=0, column=1, sticky='ns')
        ...
        initial_engine = CONFIG.get('engine', 'mymemory')
        display_engine = ENGINE_DISPLAY.get(initial_engine, initial_engine)

        self.engine_var = tk.StringVar(value=display_engine)
        engine_values = list(ENGINE_DISPLAY.values())   # 显示名称列表
        ...
        self.openai_model_var = tk.StringVar(value=CONFIG.get('openai_model', 'gpt-3.5-turbo'))
        ...
        self.claude_model_var = tk.StringVar(value=CONFIG.get('claude_model', 'claude-3-haiku-20240307'))
        ...
        self.deepseek_model_var = tk.StringVar(value=CONFIG.get('deepseek_model', 'deepseek-chat'))
        ...
        version_label = tk.Label(bottom_frame, text="版本 1.0.0", fg="#87CEFA")
        version_label.pack(side=tk.LEFT, padx=5)
        author_label = tk.Label(bottom_frame, text="作者：DeepSeek&神尾家的小観鈴", fg="#87CEFA")
        author_label.pack(side=tk.LEFT, padx=5)
        comment_label = tk.Label(bottom_frame, text="（AI 翻译仅供参考，请人工校对。开源软件，使用风险自负。）", fg="red")
    ...
    def toggle_mod_list(self):
        if self.list_frame.winfo_ismapped():
            # 折叠：隐藏列表，并取消第2行的权重，避免留白
            self.list_frame.grid_remove()
            self.root.grid_rowconfigure(2, weight=0)   # 第2行不再扩展
            self.collapse_btn.config(text="▼ 展开模组列表")
        else:
            # 展开：显示列表，并恢复第2行的权重
            self.list_frame.grid()
            self.root.grid_rowconfigure(2, weight=1)   # 第2行重新获得扩展能力
    ...
    def on_engine_change(self, event=None):
        ...
        engine = ENGINE_DISPLAY_TO_ID.get(display_engine, display_engine)  # 转为内部标识符
        # 以下保持不变，但使用 engine 变量
    ...
    def load_config(self):
        ...
        self.openai_model_var.set(CONFIG.get('OpenAi_model', 'Gpt-3.5-turbo'))
        self.claude_api_key_var.set(CONFIG.get('Claude_api_key', ''))
        self.claude_model_var.set(CONFIG.get('Claude_model', 'Claude-3-haiku-20240307'))
