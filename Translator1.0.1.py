...
CONFIG = {
    ...
    'batch_size': 60,              # AI批量翻译每批条数（降低默认值以提高稳定性）
    'mymemory_api_key': '',
    'openai_api_key': '',
...
class TranslatorApp:
    ...
    def translate_ai_batch(self, translations, target_lang, engine):
        ...
        def process_batch(batch_idx):
            ...
            # 回退到逐条（这里可以直接调用单条翻译，但注意不要递归调用自身）
                for k in batch_keys:
                    translated_value = translate_value(translations[k], target_lang)
                    with lock:
                         translated[k] = translated_value
                         if translations[k].strip() and translated_value == translations[k]:
                             failed.append(k)
            else:
            # 成功，但可能缺失键
                for k in batch_keys:
                    if k in result:
                        translated_value = result[k]
                        with lock:
                            translated[k] = translated_value
                            if translations[k].strip() and translated_value == translations[k]:
                                failed.append(k)
    ...
    # 使用线程池并发处理批次
        max_workers = min(3, total_batches)  # 限制并发数，避免速率限制
